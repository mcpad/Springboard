...
SELECT COUNT(name) FROM country_club.Facilities 
WHERE membercost = 0.0
...
FROM (SELECT CONCAT(firstname, ' ', surname) as member, f.name AS facility,
	CASE WHEN m.memid = 0 THEN b.slots*f.guestcost
		ELSE b.slots*f.membercost END AS cost
	FROM country_club.Members m
INNER JOIN country_club.Bookings b
...
SELECT name, revenue 
FROM (SELECT f.name, sum(CASE WHEN memid = 0 THEN slots * f.guestcost
			ELSE slots * membercost end) AS revenue
FROM country_club.Bookings b
        INNER JOIN country_club.Facilities f
	ON b.facid = f.facid
GROUP BY f.name) 
AS tot WHERE revenue < 1000
ORDER BY revenue;  
